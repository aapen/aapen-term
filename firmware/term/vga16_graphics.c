...
char textcolor, textbgcolor, bgcolor;
...
// Sroll (pixel) lines up by copying one part of
// the framebuffer up.
// Does the copy 8 bytes at a time.
void scrollLines(short n) {
  short offset = (640 * n) / 2;
  uint64_t *pDst = (uint64_t *)vga_data_array;
  uint64_t *pSrc = (uint64_t *)(vga_data_array + offset);

  int l = (TXCOUNT - offset) / 8;
...
// Scroll one part of the 
void scrollUp(void) {
  scrollLines(textsize * 8);
  short row = _height / (textsize * 8);
  printf("clearing line %d", row);
  clearTextLine(row);
}


void copyPixels(short xsrc, short ysrc, short xdst, short ydst, short len) {
  // Range checks (640x480 display)
  xsrc = clamp(xsrc, 0, 640);
  ysrc = clamp(ysrc, 0, 480);
  xdst = clamp(xdst, 0, 640);
  ydst = clamp(ydst, 0, 480);

  int srcByte = ((640 * ysrc) + xsrc) / 2;
  int dstByte = ((640 * ydst) + xdst) / 2;

  int nBytes = len / 2;

  for(int i = 0; i < nBytes; i++) {
    vga_data_array[srcByte+i] = vga_data_array[dstByte+i];
  }
}
...
void clearScreen() {
  fillRect(0, 0, _width-1, _height-1, bgcolor);
}


void clearTextLine(short row) {
  short y = row * textsize * 8;
  printf("filling 0 %d %d %d\n", y, _width, textsize*8);
  fillRect(0, y, _width, textsize*8, bgcolor);
}
...
inline short getCursorX(void) {
  return cursor_x;
}

inline short getCursorY(void) {
  return cursor_y;
}


inline short getRow(void) {
  return cursor_x / (textsize * 6);
}

inline short getCol(void) {
  return cursor_y / (textsize * 8);
}

inline void setTextPosition(short col, short row) {
  cursor_x = col * textsize * 6;
  cursor_y = row * textsize * 8;
}
...
inline void writeChar(unsigned char c){
  switch(c) {
    case '\n':
      ...
      break;
    case '\b':
      fillRect(cursor_x, cursor_y, 6, 8, bgcolor);
      cursor_x -= textsize*6;
